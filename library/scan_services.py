...
class ServiceScanService(BaseService):
    ...
    def gather_services(self):
        ...
        # RH sysvinit
            ...
            # print '%s --status-all | grep -E "is (running|stopped)"' %
            # service_path
            p = re.compile(
            ...
            for line in stdout.split('\n'):
                ...
                if m:
                    ...
                    service_data = {"name": service_name,
                                    "state": service_state, "source": "sysv"}
...
class SystemctlScanService(BaseService):
    ...
    def systemd_enabled(self):
        # Check if init is the systemd command, using comm as cmdline could be
        # symlink
        try:
    ...
    def gather_services(self):
        ...
        if systemctl_path is None:
            return None
        rc, stdout, stderr = self.module.run_command(
            "%s list-unit-files --type=service | tail -n +2 | head -n -2" % systemctl_path, use_unsafe_shell=True)
        for line in stdout.split("\n"):
            line_data = line.split()
            if len(line_data) != 2:
                continue
            if line_data[1] == "enabled":
                ...
                state_val = "stopped"
            services[line_data[0].replace(".service", "")] = {
                "name": line_data[0].replace(".service", ""), "state": state_val, "source": "systemd"}
        return services
